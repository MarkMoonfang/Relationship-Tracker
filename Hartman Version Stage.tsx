...
export class Stage extends StageBase<InitStateType, ChatStateType, MessageStateType, ConfigType> {
  ...
  for (const emotion of emotions) {
    ...
    for (const sub of expanded) {
      ...
      logs.push(`➕ ${umbrella} ➝ ${sub} × ${(emotion.confidence * 100).toFixed(0)}% = ${weight * emotion.confidence}`);
    }
  }
  ...
  try {
    ...
    console.error("Failed to load Hartmann model client", e);
  }
  ...
  async afterResponse(botMessage: Message): Promise<Partial<StageResponse<ChatStateType, MessageStateType>>> {
     ...
     const prediction = await this.emotionClient.predict("/predict", {
        text: content, // 👈 New key required by Hartmann model
      });

       const allEmotions: { label: string; confidence: number }[] = prediction.data.map((e: any) => ({
         label: e.label,
         confidence: typeof e.score === "number" ? e.score : parseFloat(e.score ?? "0")
        })).filter((e) => !isNaN(e.confidence));
      ...
      const filtered = allEmotions.filter(e => e.confidence >= 0.1);
       ...
       // Narrator log only (no affection)
       if (!botId || !this.charactersMap[botId] || isNarrator) {
         this.myInternalState['narratorEmotionLog'] = {
          speaker: rawName || botId || "Unknown",
           emotions: filtered,
         };
        logs.push("📎 Narrator or unknown speaker — emotion detected, affection not updated.");
    ...
    // Emotion umbrella expansion ➝ affection score
    ...
    console.warn("Hartmann emotion classification failed", err);
